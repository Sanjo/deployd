...
function Collection(name, options) {
  ...
  if(config) {
    ...
    this.enableBatchUpdating = !!config.enableBatchUpdating;
  }
  if (options) {
...
/**
 * Execute the onPut listener. If it succeeds,
...
Collection.prototype.batchUpdate = function (ctx, fn) {
  var collection = this
    , store = this.store
    , session = ctx.session
    , item = ctx.body
  ...
  if(!item) return done('You must include an object when updating.');
  ...
  if (!util.isArray(item.id)) return done ('object.id must be an array with IDs');
  query.id = {$in: item.id};
  delete item.id;

  debug('updating %j with ids %s', item, query.id);
  ...
  function put() {
    var sanitizedQuery = collection.sanitizeQuery(query)
      , prev = {};

    store.find(sanitizedQuery, function(err, result) {
      if(!obj) {
        return done(new Error("No objects exists with that ids"));
      }
      if(err) return done(err);

      async.each(result, putOne, done);
      ...
      function putOne(obj) {
        ...
        function commit(err) {
          ...
          var id = item.id;
          delete item.id;
          store.update({id: id}, item, function (err) {
            if(err) return done(err);
            item.id = id;
